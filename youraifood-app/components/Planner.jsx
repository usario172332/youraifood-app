...
// Ordered, de-duplicated recipe IDs from a generated plan (Day 1 breakfast,
// Day 1 main, Day 1 snack, Day 2 breakfast, ...) — saved to localStorage so
// the Recipes page can show "Your latest plan, in order" without the user
// having to remember dish names while browsing.
function extractOrderedRecipeIds(data) {
const mealSlots = Array.isArray(data.meals) && data.meals.length ? data.meals : ['breakfast', 'main', 'snack'];
const ids = [];
const seen = new Set();
(data.days || []).forEach((day) => {
mealSlots.forEach((slot) => {
(day[`${slot}Dishes`] || []).forEach((d) => {
...
() =>
calculateTargets({
weight: form.weight === '' ? DEFAULT_FORM.weight : form.weight,
height: form.height === '' ? DEFAULT_FORM.height : form.height,
age: form.age === '' ? DEFAULT_FORM.age : form.age,
sex: form.sex,
...
setResult(data);
try {
const ids = extractOrderedRecipeIds(data);
if (ids.length) {
localStorage.setItem('yaf_latest_plan', JSON.stringify({ ids, savedAt: Date.now() }));
}
} catch (storageErr) {
// localStorage can fail in private-browsing mode — not critical, skip silently.
}
...
onChange={(e) => { const v = e.target.value; setForm({ ...form, weight: v === '' ? '' : Number(v) }); setStatsTouched(true); }}
className="w-full rounded-lg border-[1.5px] border-gray-200 px-3 py-2.5 text-sm" />
</Field>
<Field label="Height" hint="(cm)" id="heightInput">
<input id="heightInput" type="number" min={120} max={230} step={1} value={form.height}
onChange={(e) => { const v = e.target.value; setForm({ ...form, height: v === '' ? '' : Number(v) }); setStatsTouched(true); }}
className="w-full rounded-lg border-[1.5px] border-gray-200 px-3 py-2.5 text-sm" />
</Field>
<Field label="Age" id="ageInput">
<input id="ageInput" type="number" min={14} max={100} step={1} value={form.age}
onChange={(e) => { const v = e.target.value; setForm({ ...form, age: v === '' ? '' : Number(v) }); setStatsTouched(true); }}
className="w-full rounded-lg border-[1.5px] border-gray-200 px-3 py-2.5 text-sm" />
</Field>
